...
export function SafeTime({ date, options, locale = DEFAULT_LOCALE, localize = false, className }: SafeDateProps) {
  ...
  const d = date instanceof Date ? date : new Date(date)
  const useLocal = localize && mounted
  // Force 24h format by default if hour options are provided
  const timeOptions = options?.hour ? { hour12: false, ...options } : options
  const text = (timeOptions?.dateStyle ? d.toLocaleString : d.toLocaleTimeString).call(d, useLocal ? undefined : locale, {
    timeZone: useLocal ? undefined : DEFAULT_TIME_ZONE,
    ...timeOptions,
  })
  return (
